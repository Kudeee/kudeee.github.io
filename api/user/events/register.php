 ...
 *   payment_method  string   optional  required if event fee > 0
 *   card_number     string   optional
 *   card_expiry     string   optional
 *   card_cvv        string   optional
 *
 * Response 200:
 *   { "success": true, "message": "Registered successfully!", "registration_id": 7 }
 *
 * Response 400 / 401 / 409:
 *   { "success": false, "message": "..." }
 *
 * DB tables used (when connected):
 *   events               (id, fee, max_attendees, current_attendees, is_members_only, status)
 *   event_registrations  (id, event_id, member_id, payment_method, amount_paid,
...
/*
    ...
    $stmt = $pdo->prepare('
        SELECT id, name, fee, max_attendees, current_attendees, is_members_only, status, event_date
        FROM events
        WHERE id = ?
        LIMIT 1
    ');
