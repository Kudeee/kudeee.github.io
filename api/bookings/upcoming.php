<?php
require_once __DIR__ . '/../../../api/config.php';

$member = require_member();
$pdo    = db();

// Get the next upcoming confirmed class booking
$stmt = $pdo->prepare("
    SELECT
        cb.id                                       AS booking_id,
        'class'                                     AS booking_type,
        cb.class_name,
        cb.booking_date,
        cb.booking_time,
        cb.status,
        CONCAT(t.first_name, ' ', t.last_name)      AS trainer_name,
        cs.duration_minutes,
        cs.scheduled_at,
        cb.class_schedule_id
    FROM class_bookings cb
    LEFT JOIN class_schedules cs ON cs.id = cb.class_schedule_id
    LEFT JOIN trainers t         ON t.id  = cs.trainer_id
    WHERE cb.member_id = ?
      AND cb.status    = 'confirmed'
      AND (
            cb.booking_date > CURDATE()
         OR (cb.booking_date = CURDATE() AND cb.booking_time >= CURTIME())
      )
    ORDER BY cb.booking_date ASC, cb.booking_time ASC
    LIMIT 1
");
$stmt->execute([$member['id']]);
$nextClass = $stmt->fetch();

// Fallback: also check trainer bookings
$stmt2 = $pdo->prepare("
    SELECT
        tb.id                                       AS booking_id,
        'trainer'                                   AS booking_type,
        CONCAT('Session w/ ', t.first_name)         AS class_name,
        tb.booking_date,
        tb.booking_time,
        tb.status,
        CONCAT(t.first_name, ' ', t.last_name)      AS trainer_name,
        tb.session_minutes                           AS duration_minutes,
        NULL                                         AS scheduled_at,
        NULL                                         AS class_schedule_id
    FROM trainer_bookings tb
    JOIN trainers t ON t.id = tb.trainer_id
    WHERE tb.member_id = ?
      AND tb.status    = 'confirmed'
      AND (
            tb.booking_date > CURDATE()
         OR (tb.booking_date = CURDATE() AND tb.booking_time >= CURTIME())
      )
    ORDER BY tb.booking_date ASC, tb.booking_time ASC
    LIMIT 1
");
$stmt2->execute([$member['id']]);
$nextTrainer = $stmt2->fetch();

// Pick whichever is sooner
$next = null;
if ($nextClass && $nextTrainer) {
    $classTS   = strtotime($nextClass['booking_date']   . ' ' . $nextClass['booking_time']);
    $trainerTS = strtotime($nextTrainer['booking_date'] . ' ' . $nextTrainer['booking_time']);
    $next = ($classTS <= $trainerTS) ? $nextClass : $nextTrainer;
} elseif ($nextClass) {
    $next = $nextClass;
} elseif ($nextTrainer) {
    $next = $nextTrainer;
}

if ($next) {
    // Format date label
    $bookingDate = new DateTime($next['booking_date']);
    $today       = new DateTime('today');
    $tomorrow    = new DateTime('tomorrow');

    if ($bookingDate == $today) {
        $next['date_label'] = 'Today';
    } elseif ($bookingDate == $tomorrow) {
        $next['date_label'] = 'Tomorrow';
    } else {
        $next['date_label'] = $bookingDate->format('M j, Y');
    }

    // Format time label (e.g. "9:00 AM")
    $next['time_label'] = date('g:i A', strtotime($next['booking_time']));

    // Duration label
    $next['duration_label'] = $next['duration_minutes'] ? $next['duration_minutes'] . ' min' : '50 min';
}

success('OK', ['next_booking' => $next]);